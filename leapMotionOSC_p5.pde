...
void setup() {
  ...
  oscP5 = new OscP5(this, 44001); // receive port is not used
...
void draw() {
  ...
  for (Hand hand : leap.getHands ()) {
    ...
    //populate and send the OSC message
    String handType = hand.isLeft() ? "Left":"Right";    //pretty sweet way of knowing which hand
    OscMessage message = new OscMessage("/"+ handType);
    message.add(0);
    oscP5.send(message,HOST,PORT);
